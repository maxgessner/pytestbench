...
r_mean = voltage_drop / mcurrent * s / length
temp_mean = np.mean(scan[:, :, 1])
...
temp_0 = np.average(scan[:, :, 1], weights=x, axis=1)
# temp_0 = scan[:, :, 1]  #* x / np.mean(x)

# generate R_0 depending on slice width
# R_0 = (voltage_drop / mcurrent / np.shape(scan)[1]) * x / s
# R_0 = r_mean * x / s
R_0 = np.empty([np.shape(r_mean)[0], np.shape(x)[0]])
for val in range(np.shape(r_mean)[0]):
    R_0[val, :] = r_mean[val] * x[:] / s
...
# print(cf_model(scan[:, :, 1], 0.004))
# boundary conditions
R_ges = voltage_drop / mcurrent
...
if fit_method == 'curve_fit':
    ...
    '''
    popt, pcov = curve_fit(cf_model, np.mean(scan[:, :, 0]), R_0,
                           # bounds=(0, 0.1),
                           p0=alpha_0,
                           absolute_sigma=True,
                           # method='trf',
    ...
    popt, pcov = curve_fit(cf_model, scan[:, :, 1], R_0,
...
if fit_method == 'least_squares':
    ...
    res = least_squares(residual, alpha_0, args=(scan[:, :, 1], R_ges),
                        verbose=2,
                        bounds=(-0.01, 0.01),
...
if fit_method == 'minimize_scalar':
    ...
    res = minimize_scalar(residual_2,  # args=(scan[:, :, 1], R_ges),
                          bounds=(0, 0.01),
                          # bracket=(0.01, 0.001),
                          # tol=1e-10,
                          method='bounded',
...
if fit_method == 'fminbound':
    ...
    # '''
    res = fminbound(residual, 0, 0.01, args=(scan[:, :, 1], R_ges),
                    disp=3,
                    maxfun=1000,
                    # retall=True,
                    )
...
# r = f_rho(temp_0) * (1 + (value * (scan[:, :, 1] - temp_0)))
r = r_mean * (1 + (value * (scan[:, :, 1] - temp_mean)))
# r = np.sum(R_0) * s / length * (1 + (value * (scan[:, :, 1] - temp_0)))

# generate temperature range from profile
temp_range = np.linspace(np.min(scan[:, :, 1]), np.max(scan[:, :, 1]), 1000)
# temp_range = np.linspace(t_a, np.max(scan[:, :, 1]), 1000)
